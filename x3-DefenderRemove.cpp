...
int main(int argc, char** argv) {
	...
	printf("Windows Defender卸载工具  v2.0\n");
	...
	system("reg add \"HKLM\\SYSTEM\\CurrentControlSet\\Services\\SecurityHealthService\" /v Start /t REG_DWORD /d 4 /f");//注册表禁用（可能是代码中对wd最温柔的了(迫真
	...
	if(CloseService("WdNisDrv"))printf("[-] 停止服务WdNisDrv(Windows 安全中心服务)失败\n"); 
	else printf("[+] 停止服务SecurityHealthService(Windows 安全中心服务)成功\n"); 
